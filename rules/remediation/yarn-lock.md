# Remédiation — Exposition yarn.lock en webroot public
...
```gitignore
# yarn.lock est normalement versionné volontairement ; s'assurer qu'il n'est jamais copié dans le webroot public de déploiement
```

## Vérification CI/CD (avant déploiement)
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : yarn.lock ne doit pas exister sous le répertoire servi publiquement à l'issue du build.

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name "yarn.lock" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (yarn.lock) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
