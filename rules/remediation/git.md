# Remédiation — Exposition .git/ en webroot public
...
```gitignore
# Ne s'applique pas à .gitignore : le répertoire .git ne doit simplement jamais être copié dans le webroot de déploiement
```

## Vérification CI/CD (avant déploiement)
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : échouer le déploiement si un répertoire .git/ est détecté sous le webroot (find /var/www/html -maxdepth 2 -name .git).

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name ".git/ (répertoire complet)" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (.git/ (répertoire complet)) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
