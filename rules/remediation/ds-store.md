# Remédiation — Exposition .DS_Store en webroot public
...
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : aucun fichier .DS_Store ne doit être présent dans l'artefact déployé (find . -name '.DS_Store' doit renvoyer vide).

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name ".DS_Store" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (.DS_Store) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
