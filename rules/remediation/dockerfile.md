# Remédiation — Exposition Dockerfile en webroot public
...
```gitignore
# Dockerfile est normalement versionné volontairement ; s'assurer qu'il n'est jamais copié dans l'image finale ni le webroot
```

## Vérification CI/CD (avant déploiement)
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : vérifier via un multi-stage build que le Dockerfile lui-même et les ARG contenant des secrets ne se retrouvent pas dans l'image finale ni sous le webroot.

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name "Dockerfile" -o -name "Dockerfile.*" -o -name "*.dockerfile" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (Dockerfile, Dockerfile.*, *.dockerfile) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
