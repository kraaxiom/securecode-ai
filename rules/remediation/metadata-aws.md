# Remédiation — SSRF vers le service de métadonnées AWS (IMDS)
...
## Note spécifique — Métadonnées AWS (IMDS)
Filtrer explicitement l'adresse `169.254.169.254` (et le nom d'hôte de métadonnées le cas échéant) dans toute logique de validation des destinations de requêtes sortantes, en plus de la whitelist générale. Endpoint sensible : `169.254.169.254 (IMDSv1/IMDSv2)`. Côté infrastructure, appliquer le principe du moindre privilège sur le rôle/l'identité/le compte de service attaché à la ressource, et privilégier la variante la plus sécurisée du service de métadonnées quand elle existe (ex: IMDSv2 pour AWS).
