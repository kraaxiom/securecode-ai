# Remédiation — Exposition package-lock.json en webroot public
...
```gitignore
# package-lock.json est normalement versionné volontairement ; s'assurer qu'il n'est jamais copié dans le webroot public de déploiement
```

## Vérification CI/CD (avant déploiement)
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : package-lock.json/package.json ne doivent pas exister sous le répertoire servi publiquement à l'issue du build.

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name "package-lock.json" -o -name "package.json" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (package-lock.json, package.json) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
