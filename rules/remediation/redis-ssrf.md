# Remédiation — SSRF ciblant Redis
...
## Note spécifique — Redis
Activer `requirepass`/ACL sur toute instance Redis, même en réseau interne, et la lier à une interface non exposée publiquement (`bind 127.0.0.1` ou interface privée uniquement). Ne jamais permettre à une requête HTTP contrôlée par l'utilisateur d'atteindre le port Redis (6379 par défaut) via une SSRF classique ou un proxy générique.
