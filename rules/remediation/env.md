# Remédiation — Exposition .env en webroot public
...
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : aucun fichier .env* (hors .env.example) ne doit être présent dans le répertoire déployé comme webroot.

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name ".env" -o -name ".env.local" -o -name ".env.production" -o -name ".env.*" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (.env, .env.local, .env.production, .env.*) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
