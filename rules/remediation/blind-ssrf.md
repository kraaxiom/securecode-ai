...
## Note spécifique — SSRF aveugle
Même sans retour visible au client, appliquer strictement les mêmes contrôles (whitelist, validation IP, timeout). Journaliser les requêtes sortantes émises par les jobs asynchrones/workers pour permettre la détection a posteriori d'un scan interne, et désactiver la résolution d'entités externes dans les parseurs XML/documents utilisés par ces mêmes traitements.
