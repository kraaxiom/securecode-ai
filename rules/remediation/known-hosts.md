# Remédiation — Exposition known_hosts en webroot public
...
Ajouter une étape de pipeline qui échoue le build si le motif suivant est détecté dans l'artefact de déploiement : aucun fichier known_hosts ne doit être présent dans l'artefact déployé sous le webroot.

Exemple générique (à adapter au CI utilisé) :
```bash
if find ./dist ./public ./build -type f \( -name "known_hosts" \) 2>/dev/null | grep -q .; then
...
- [ ] Le fichier (known_hosts) n'est plus atteignable par une requête HTTP directe (test : `curl -I https://exemple.tld/<fichier>` doit renvoyer 403/404).
